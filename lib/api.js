...
class ApiClient {
  ...
  async request(path, options = {}) {
    ...
    try {
      ...
      // Handle token expiry / auth failure
      if (res.status === 401) {
        const data = await res.json();

        // Try to refresh if token expired and we have a refresh token
        ...
        // Only clear tokens if we actually HAD a token and it was rejected
        // (not just a "No token provided" for a page that checks auth)
        if (this.accessToken && data.code !== 'TOKEN_EXPIRED') {
          this.clearTokens();
          if (typeof window !== 'undefined') {
            localStorage.removeItem('user');
          }
  ...
  // Orders
  getOrders(page = 1) { return this.request(`/orders/my?page=${page}`); }
  ...
  getGroupMembers(id) { return this.request(`/groups/${id}/members`); }
  verifyGroupMember(groupId, userId, verified) {
    return this.request(`/groups/${groupId}/members/${userId}/verify`, {
      method: 'PATCH', body: JSON.stringify({ verified }),
    });
  }
  removeGroupMember(groupId, userId) {
    return this.request(`/groups/${groupId}/members/${userId}/remove`, { method: 'POST' });
  }
  ...
  adminGetPlans(params = {}) {
    const q = new URLSearchParams(params).toString();
    return this.request(`/plans${q ? '?' + q : ''}`);
  }
  adminLinkPlanGroup(planId, groupId) {
    return this.request(`/plans/${planId}/link-group`, { method: 'PATCH', body: JSON.stringify({ group_id: groupId }) });
  }
