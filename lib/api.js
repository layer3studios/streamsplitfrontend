...
class ApiClient {
  ...
  resolveInvite(code) { return this.request(`/groups/invite/${encodeURIComponent(code.trim().toUpperCase())}`); }
  initiateInviteJoin(code, payload = {}) {
    return this.request(`/groups/invite/${encodeURIComponent(code.trim().toUpperCase())}/join/initiate`, {
      method: 'POST', body: JSON.stringify(payload),
    });
  }
  confirmInviteJoin(code, joinIntentId) {
    return this.request(`/groups/invite/${encodeURIComponent(code.trim().toUpperCase())}/join/confirm`, {
      method: 'POST', body: JSON.stringify({ joinIntentId }),
    });
  }
