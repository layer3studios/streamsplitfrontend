    ...
    return (
        <div>
            <div className="flex items-center justify-between mb-6">
                <div>
                    <h1 className="font-heading font-bold text-2xl text-[var(--text)]">Brand Management</h1>
                    <p className="text-[var(--muted)] text-sm mt-1">{brands.length} brands</p>
                </div>
            ...
            {showForm && (
                <form onSubmit={handleCreate} className="card p-5 mb-6 hover:transform-none animate-slide-up">
                    <h3 className="font-heading font-semibold text-[var(--text)] mb-4">New Brand</h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                        <div>
                            <label className="text-[var(--muted)] text-xs font-medium mb-1 block">Name *</label>
                        ...
                        <div>
                            <label className="text-[var(--muted)] text-xs font-medium mb-1 block">Slug *</label>
                            <input value={form.slug} onChange={(e) => setForm({ ...form, slug: e.target.value })}
                                placeholder="netflix" className="input py-2.5" required />
                        </div>
                        <div>
                            <label className="text-[var(--muted)] text-xs font-medium mb-1 block">Category *</label>
                        ...
                        <div>
                            <label className="text-[var(--muted)] text-xs font-medium mb-1 block">Logo URL</label>
                            <input value={form.logo_url} onChange={(e) => setForm({ ...form, logo_url: e.target.value })}
                                placeholder="/images/brands/logo.png" className="input py-2.5" />
                        </div>
                        <div className="sm:col-span-2">
                            <label className="text-[var(--muted)] text-xs font-medium mb-1 block">Description</label>
                    ...
                    <div className="flex items-center gap-2 mt-3">
                        ...
                        <label htmlFor="featured" className="text-[var(--muted)] text-sm">Featured brand</label>
                    </div>
                    ...
                    <table className="w-full">
                        <thead>
                            <tr className="border-b border-[var(--border)]">
                                ...
                                <th className="text-left text-[var(--muted)] text-xs font-medium uppercase py-3 px-4">Category</th>
                                <th className="text-left text-[var(--muted)] text-xs font-medium uppercase py-3 px-4">Plans</th>
                                <th className="text-left text-[var(--muted)] text-xs font-medium uppercase py-3 px-4">Featured</th>
                        ...
                        <tbody>
                            {loading ? (
                                ...
                                <tr key={brand._id} className="border-b border-[var(--border)]/50 hover:bg-white/2 transition-all">
                                        ...
                                        <div className="flex items-center gap-2.5">
                                            <div className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center overflow-hidden">
                                                {brand.logo_url ? (
                                                    ...
                                                    <span className="text-[var(--muted)] text-xs font-bold">{brand.name.charAt(0)}</span>
                                                )}
                                            </div>
                                            <div>
                                                <span className="text-[var(--text)] text-sm font-medium">{brand.name}</span>
                                                <p className="text-[var(--muted)] text-xs">{brand.slug}</p>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="py-3 px-4 text-[var(--muted)] text-sm">{brand.category_id?.name || '—'}</td>
                                    <td className="py-3 px-4 text-[var(--text)] text-sm">{brand.plan_count}</td>
                                    <td className="py-3 px-4">
                                        <button onClick={() => handleToggleFeatured(brand)} disabled={actionLoading === brand._id + '_feat'}>
                                            {brand.is_featured ? <Star className="w-5 h-5 text-yellow-400 fill-yellow-400" /> : <Star className="w-5 h-5 text-[var(--muted)]" />}
                                        </button>
                                    </td>
                                    <td className="py-3 px-4">
                                        <button onClick={() => handleToggleActive(brand)} disabled={actionLoading === brand._id + '_act'}>
                                            {brand.is_active !== false ? <ToggleRight className="w-6 h-6 text-green-400" /> : <ToggleLeft className="w-6 h-6 text-[var(--muted)]" />}
                                        </button>
                                    </td>
                                </tr>
                            ))}
