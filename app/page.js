...
import FeaturedBrandsSection from '../components/featured/FeaturedBrandsSection';
  ...
  return (
    <div className="min-h-screen">
      ...
      <MotionPage>
        ...
        <section className="pt-28 pb-16 md:pt-36 md:pb-24">
          <Container className="text-center">
            ...
            <div className="flex items-center justify-center gap-3 mt-8">
              ...
              {!isAuthenticated && (
                <Link href="/join" className="btn-secondary">Join via Invite</Link>
              )}
            </div>
          </Container>
        </section>
          ...
          <Container>
            ...
            <div className="flex flex-wrap justify-center gap-2 mt-10">
              {categories.length > 0 ? categories.map(cat => (
                <Link key={cat._id || cat.name} href={`/explore?category=${cat.slug || cat.name}`}
                  className="badge hover:border-[var(--border2)] transition-colors">{cat.name}</Link>
              )) : (
                Array.from({ length: 6 }).map((_, i) => <div key={i} className="skeleton w-20 h-7 rounded-full" />)
              )}
            </div>
          </Container>
        </section>
        ...
        {/* ═══ FEATURED BRANDS — editorial stack ═══ */}
        {loading ? (
          <section className="py-[var(--section-gap)]"><Container><GridSkeleton count={6} /></Container></section>
        ) : (
          <FeaturedBrandsSection brands={brands} groups={groups} />
        )}
                ...
                return (
                  <div key={group._id} className="paper-card p-5">
                    ...
                    <p className="text-[9px] tracking-wider uppercase text-[var(--muted)] opacity-60 mb-3">
                      {group.brand_id?.name || group.plan_id?.brand_name || 'Service'}
                    </p>
          ...
          <Container>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Link href="/create-group" className="card-dark group">
                ...
                <p className="text-sm text-[var(--ctaText)] opacity-60 mb-6">Share your subscription, earn back your costs.</p>
              ...
              <Link href="/join" className="card-dark group">
                ...
                <p className="text-sm text-[var(--ctaText)] opacity-60 mb-6">Got an invite? Enter your code and join instantly.</p>
                <div className="inline-flex items-center gap-2 text-sm text-[var(--ctaText)] font-medium group-hover:gap-3 transition-all">
