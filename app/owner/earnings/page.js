            ...
            <div className="min-h-screen">
                ...
                <main className="pt-20 pb-24 px-4 flex items-center justify-center min-h-[60vh]">
                    <div className="text-center">
                        ...
                        <h2 className="font-heading font-bold text-xl text-white mb-2">Sign in to view earnings</h2>
            ...
            <main className="pt-20 pb-24 md:pb-8 px-4 lg:px-6">
                <div className="max-w-3xl mx-auto">
                    <div className="flex items-center justify-between mb-6">
                        <h1 className="font-heading font-bold text-2xl text-white">My Earnings</h1>
                    ...
                    {loading ? (
                        ...
                        <div className="card p-8 text-center">
                            <p className="text-gray-500">No earnings yet. Create a group and share the invite link!</p>
                        ...
                        <>
                            ...
                            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
                                <div className="card p-4 hover:transform-none">
                                    ...
                                    <p className="text-white font-heading font-bold text-xl">{BRAND.currency.symbol}{summary.withdrawable_balance}</p>
                                    <p className="text-gray-500 text-xs">Withdrawable</p>
                                </div>
                                <div className="card p-4 hover:transform-none">
                                    <TrendingUp className="w-5 h-5 text-brand-primary-light mb-1" />
                                    <p className="text-white font-heading font-bold text-xl">{BRAND.currency.symbol}{summary.total_earned}</p>
                                    <p className="text-gray-500 text-xs">Total Earned</p>
                                </div>
                                <div className="card p-4 hover:transform-none">
                                    <Users className="w-5 h-5 text-blue-400 mb-1" />
                                    <p className="text-white font-heading font-bold text-xl">{summary.owned_groups}</p>
                                    <p className="text-gray-500 text-xs">Groups Owned</p>
                                </div>
                                <div className="card p-4 hover:transform-none">
                                    <ArrowDownRight className="w-5 h-5 text-yellow-400 mb-1" />
                                    <p className="text-white font-heading font-bold text-xl">{BRAND.currency.symbol}{summary.pending_withdrawals}</p>
                                    <p className="text-gray-500 text-xs">Pending Withdrawals</p>
                                </div>
                            </div>

                            {/* Fee info */}
                            <div className="bg-dark-surface/50 rounded-xl p-3 mb-6 text-xs text-gray-500 flex items-center justify-between">
                            ...
                            {/* Transaction Ledger */}
                            <h2 className="font-heading font-bold text-lg text-white mb-3">Transaction History</h2>
                            {txns.length === 0 ? (
                                <div className="card p-6 text-center text-gray-500 text-sm">No transactions yet</div>
                            ) : (
                                    ...
                                    {txns.map(tx => (
                                        <div key={tx._id} className="card p-4 hover:transform-none">
                                            <div className="flex items-center justify-between mb-1">
                                                <div>
                                                    <p className="text-white text-sm font-medium">{tx.group_id?.name || 'Group'}</p>
                                                    <p className="text-gray-600 text-xs">Buyer: {tx.buyer_id?.name || tx.buyer_id?.phone || 'Unknown'}</p>
                                                </div>
